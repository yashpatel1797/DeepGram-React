...
export const followUser = createAsyncThunk(
    ...
    async (followUserId) => {
        try {
            const { data: { followUser, user: { followers, following }, }, } = await axios.post(`/api/users/follow/${followUserId}`);
...
export const unFollowUser = createAsyncThunk(
    ...
    async (followingUserId) => {
        try {
            const { data: { followUser, user: { followers, following }, }, } = await axios.post(`/api/users/unfollow/${followingUserId}`);
            return { followUser, followers, following };
        } catch (error) {
